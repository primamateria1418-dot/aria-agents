...
class ResearchAgent(BaseAgent):
    ...
    def _build_topics(self, extra_topics: list, urgent_flags: list) -> list:
        """Build the list of topics to scan today."""
        base_topics = self._get_base_topics()

        # Add peer-requested topics from urgent flags
    ...
    def _parse_topic_instructions(self, instructions: list) -> list:
        """Extract topic additions from CEO instructions."""
        topics = []
        for inst in instructions:
            text = inst.get("instruction", "").lower()
            if "research" in text or "scan" in text or "topic" in text:
                # Extract the subject after "research" or "scan"
    ...
    def _search(self, query: str) -> list:
        """Try Tavily first, fall back to SearXNG."""
        if TAVILY_KEY:
    ...
    def _extract_angles(self, topic: str, results: list) -> list:
        """Use LLM to extract content angles from search results."""
        if not results:
            return []
...
[
  {{
    ...
    def _score_angles(self, angles: list) -> list:
        """Score all angles in one LLM call for efficiency."""
        if not angles:
            return []

        angle_list = json.dumps([
            {"headline": a.get("headline"), "angle": a.get("angle"), "urgency": a.get("urgency")}
            for a in angles
        ], indent=2)
...
[{{"score": 8}}, {{"score": 6}}, ...]"""
    ...
    def _store_angle(self, angle: dict) -> dict | None:
        """Store angle to content_queue as a draft seed."""
...
if __name__ == "__main__":
    ...
    if "--test" in sys.argv:
        ...
        print("Skipping live search. Running self_review() with dummy data.")
        ...
        print("✓ Test complete — check agent_memory in Supabase")
    else:
